...
time = 5;
options = odeset('RelTol',1e-1,'AbsTol',1e-2*ones(1,4)); % solver options
% again for part3- add modeling error

[T,Y] = ode45(@(t,y) Controller_with_error(t,y,m1,m2,l1,l2,I1,I2,a,b,Kp,Kd),[0 time],ic,options); % Solve ODE
...
% plot video
plotTracking(l1,l2,y,time,t_int,a,b,'Control with 5% standard error of sensor'); 
...
figure;
%plot
suptitle('Simulation with sensor error');
subplot(2,2,1);
...
subplot(2,2,4);
plot(T,q1_error);hold on; plot(T,q2_error,'--'); hold off; xlabel('time/[s]'); ylabel('error/rad'); title('q1,q2 error with random error');
